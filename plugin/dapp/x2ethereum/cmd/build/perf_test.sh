...
loop_send_lock_eth() {
    ...
    preChainBalance=$(${ChainCli} x2ethereum balance -s 12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv -t eth | jq ".res" | jq ".[]" | jq ".balance" | sed 's/\"//g')
    ...
    nowChainBalance=$(${ChainCli} x2ethereum balance -s 12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv -t eth | jq ".res" | jq ".[]" | jq ".balance" | sed 's/\"//g')
    diff=$(echo "$nowChainBalance - $preChainBalance" | bc)
    check_number "${diff}" 7
}
...
loop_send_burn_eth() {
    ...
    while [[ i -lt ${#privateKeys[@]} ]]; do
        preEthBalance[$i]=$(curl -ksd '{"jsonrpc":"2.0","method":"eth_getBalance","params":["'${ethAddress[i]}'", "latest"],"id":1}' "${ethUrl}" | jq -r ".result")
        ethTxHash=$(${ChainCli} send x2ethereum burn -a 1 -r ${ethAddress[i]} -t eth --node_addr "${ethUrl}" -k 12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv)
        echo ${i} "burn chain tx hash:" "${ethTxHash}"
        # shellcheck disable=SC2219
        let i++
    done
    ...
    nowChainBalance=$(${ChainCli} x2ethereum balance -s 12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv -t eth | jq ".res" | jq ".[]" | jq ".balance" | sed 's/\"//g')
    diff=$(echo "$preChainBalance - $nowChainBalance" | bc)
    check_number "${diff}" 7
}
...
loop_send_lock_bty() {
    ...
    preChainBalance=$(${ChainCli} account balance -a 12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv -e x2ethereum | jq -r ".balance" | sed 's/\"//g')

    i=0
    while [[ i -lt ${#privateKeys[@]} ]]; do
        preEthBalance[$i]=$(${CLIA} relayer ethereum balance -o "${ethAddress[i]}" -t "${tokenAddrBty}" | jq -r ".balance")
        ethTxHash=$(${ChainCli} send x2ethereum lock -q "${tokenAddrBty}" -a 1 -r ${ethAddress[i]} -t coins.bty --node_addr "${ethUrl}" -k 12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv)
        echo ${i} "lock chain tx hash:" "${ethTxHash}"
        # shellcheck disable=SC2219
        let i++
    done
    ...
    nowChainBalance=$(${ChainCli} account balance -a 12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv -e x2ethereum | jq -r ".balance" | sed 's/\"//g')
    diff=$(echo "$preChainBalance - $nowChainBalance" | bc)
    check_number "${diff}" 7
}
...
loop_send_lock_erc20() {
    ...
    preChainBalance=$(${ChainCli} x2ethereum balance -s 12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv -t testc | jq ".res" | jq ".[]" | jq ".balance" | sed 's/\"//g')
    ...
    nowChainBalance=$(${ChainCli} x2ethereum balance -s 12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv -t testc | jq ".res" | jq ".[]" | jq ".balance" | sed 's/\"//g')
    diff=$((nowChainBalance - preChainBalance))
    check_number "${diff}" 7
}
...
loop_send_burn_erc20() {
    ...
    while [[ i -lt ${#privateKeys[@]} ]]; do
        ...
        ethTxHash=$(${ChainCli} send x2ethereum burn -a 1 -r ${ethAddress[i]} -t testc -q "${tokenAddr}" --node_addr "${ethUrl}" -k 12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv)
        echo ${i} "burn chain tx hash:" "${ethTxHash}"
        # shellcheck disable=SC2219
        let i++
    done
