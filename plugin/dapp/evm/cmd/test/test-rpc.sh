...
function evm_createContract() {
    ...
    chain_SignAndSendTx "${tx}" "${evm_creatorAddr_key}" "$MAIN_HTTP" "${expire}" "${gas}"
    txHash=$RAW_TX_HASH
...
function evm_callQuery() {
    ...
    chain_Http "$req" "${MAIN_HTTP}" '(.result != null)' "GetPackData" ".result.packData"
    echo "$RETURN_RESP"
    ...
    chain_Http "$req" "${MAIN_HTTP}" '(.result != null)' "Query" ".result.rawData"
    echo "$RETURN_RESP"

    req='{"method":"Chain.Query","params":[{"execer":"evm","funcName":"GetUnpackData","payload":{"abi":"'${erc20_abi}'","name":"'${name}'","data":"'${RETURN_RESP}'"}}]}'
    chain_Http "$req" "${MAIN_HTTP}" '(.result != null)' "GetUnpackData" ".result.unpackData[0]"
    echo "$RETURN_RESP"
...
function evm_addressCheck() {
    ...
    chain_Http "$req" "${MAIN_HTTP}" "${resok}" "CheckAddrExists"
...
function init() {
    ...
    chain_ImportPrivkey "${evm_creatorAddr_key}" "${evm_creatorAddr}" "evm_test" "${main_ip}"

    local from="${evm_creatorAddr}"
    local evm_addr=""

    if [ "$ispara" == false ]; then
        chain_applyCoins "$from" 12000000000 "${main_ip}"
        chain_QueryBalance "${from}" "$main_ip"

        evm_addr=$(curl -ksd '{"method":"Chain.ConvertExectoAddr","params":[{"execname":"evm"}]}' "${MAIN_HTTP}" | jq -r ".result")
    else
        paraName="user.p.para."

        chain_applyCoins "$from" 1000000000 "${main_ip}"
        chain_QueryBalance "${from}" "$main_ip"

        local para_ip="${MAIN_HTTP}"
        chain_ImportPrivkey "${evm_creatorAddr_key}" "${evm_creatorAddr}" "evm_para_test" "$para_ip"

        chain_applyCoins "$from" 12000000000 "${para_ip}"
        chain_QueryBalance "${from}" "$para_ip"

        evm_addr=$(curl -ksd '{"method":"Chain.ConvertExectoAddr","params":[{"execname":"user.p.para.evm"}]}' "${MAIN_HTTP}" | jq -r ".result")
        chain_SendToAddress "$from" "$evm_addr" 10000000000 "$MAIN_HTTP"
    fi
