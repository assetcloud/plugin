    ...
    while true; do
        ...
        if [ "${new_height}" -lt "${expect}" ]; then
            ...
            for ((k = 0; k < ${#names[@]}; k++)); do
                sync_status=$(docker exec "${names[$k]}" /root/chain-cli net is_sync)
...
function SendToPrivacyExec() {
    ...
    note=$4
    amount=$5
    #sudo docker exec -it $name ./chain-cli send coins transfer -k $fromAdd -t $execAdd -n $note -a $amount
...
function pub2priv() {
    ...
    amount=$5
    expire=$6
    #sudo docker exec -it $name ./chain-cli privacy pub2priv -f $fromAdd -p $priAdd -a $amount -n $note --expire $expire
...
function priv2priv() {
    ...
    mixcount=$6
    expire=$7
    #sudo docker exec -it $name ./chain-cli privacy priv2priv -f $fromAdd -p $priAdd -a $amount -n $note --expire $expire
...
function priv2pub() {
    ...
    mixcount=$6
    expire=$7
    #sudo docker exec -it $name ./chain-cli privacy priv2pub -f $fromAdd -t $toAdd -a $amount -n $note -m $mixcount --expire $expire
