...
function updateConfig() {
    ...
    chain_SignAndSendTx "${unsignedTx}" "0x4257d8692ef7fe13c68b65d6a52f03933db2fa5ce8faf210b5b8b80c721ced01" "${MAIN_HTTP}"
}
...
function token_sendExec() {
    ...
    chain_SignAndSendTx "${unsignedTx}" "$3" "${MAIN_HTTP}"
}
...
retrieve_Backup() {
    ...
    chain_SignAndSendTxWait "$tx" "$retrieve2_key" ${MAIN_HTTP} "$FUNCNAME"
}

retrieve_Prepare() {
    local req='{"method":"retrieve.CreateRawRetrievePrepareTx","params":[{"backupAddr":"'$retrieve1'","defaultAddr":"'$retrieve2'"}]}'
    tx=$(curl -ksd "$req" ${MAIN_HTTP} | jq -r ".result")
    chain_SignAndSendTxWait "$tx" "$retrieve1_key" ${MAIN_HTTP} "$FUNCNAME"
}
...
retrieve_QueryResult() {
    ...
    chain_Http "$req" ${MAIN_HTTP} '(.result.status == '"$status"')' "$FUNCNAME"
}
...
init() {
    ...
    chain_ImportPrivkey "${retrieve1_key}" "${retrieve1}" "retrieve1" "${MAIN_HTTP}"
    chain_applyCoins "${retrieve1}" 10000000000 "${MAIN_HTTP}"
    ...
    chain_ImportPrivkey "${retrieve2_key}" "${retrieve2}" "retrieve2" "${MAIN_HTTP}"
    chain_applyCoins "${retrieve2}" 10000000000 "${MAIN_HTTP}"

    if [ "$ispara" == true ]; then
        local main_ip=${MAIN_HTTP//8901/8801}
        chain_applyCoins "${retrieve1}" 1000000000 "${main_ip}"
        chain_applyCoins "${retrieve2}" 1000000000 "${main_ip}"
    fi

    chain_SendToAddress "$retrieve1" "$retrieve_addr" 1000000000 ${MAIN_HTTP}
    chain_SendToAddress "$retrieve2" "$retrieve_addr" 1000000000 ${MAIN_HTTP}
    symbol="RETRIEVE"
...
function main() {
    chain_RpcTestBegin retrieve
    MAIN_HTTP="$1"
    echo "ip=$MAIN_HTTP"

    init
    run_test
    chain_RpcTestRst retrieve "$CASE_ERR"
}
