...
func (a *Action) ContractToTree(payload *zt.ZkContractToTree) (*types.Receipt, error) {
	...
	////因为chain合约精度为1e8,而外部输入精度则为1e18, 单位为wei,需要统一转化为1e8
...
func (a *Action) transferToNewProcess(accountIdFrom uint64, toChainAddress, toEthAddress, totalAmount, amount string, tokenID uint64) (*types.Receipt, error) {
...
func (a *Action) transferToNewInnerProcess(fromLeaf *zt.Leaf, toChainAddress, toEthAddress, totalAmount, amount string, tokenID uint64) (*types.Receipt, error) {
	var kvs []*types.KeyValue
	var logs []*types.ReceiptLog
...
func (a *Action) TransferToNew(payload *zt.ZkTransferToNew) (*types.Receipt, error) {
	...
	//转换10进制
	newAddr, ok := zt.HexAddr2Decimal(payload.ToChainAddress)
	if !ok {
		return nil, errors.Wrapf(types.ErrInvalidParam, "transfer ToChainAddress=%s", payload.ToChainAddress)
	}
	payload.ToChainAddress = newAddr
	...
	receipts, err := a.transferToNewInnerProcess(fromLeaf, payload.GetToChainAddress(), payload.GetToEthAddress(), amountPlusFee, payload.Amount, payload.GetTokenId())
	if nil != err {
		return nil, err
	}
...
func isInvalidTx(cfg *types.ChainConfig, txHash []byte) bool {
	invalidTx, _ := getCfgInvalidTx(cfg)
	return invalidTx == hex.EncodeToString(txHash)
}

func getCfgInvalidTx(cfg *types.ChainConfig) (string, string) {
...
func isInvalidProof(cfg *types.ChainConfig, rootHash string) bool {
