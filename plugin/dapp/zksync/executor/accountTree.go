...
func getInitTreeRoot(cfg *types.ChainConfig, ethAddrDecimal, chainAddrDecimal string) string {
	var feeEth, fee33 string
	if len(ethAddrDecimal) > 0 && len(chainAddrDecimal) > 0 {
		feeEth, fee33 = ethAddrDecimal, chainAddrDecimal
	} else {
...
func AddNewLeafOpt(statedb dbm.KV, ethAddress string, tokenId, accountId uint64, amount string, chainAddr string) ([]*types.KeyValue, error) {
	var kvs []*types.KeyValue
...
func applyL2AccountCreate(accountID, tokenID uint64, amount, ethAddress, chainAddr string, statedb dbm.KV, makeEncode bool) ([]*types.KeyValue, *types.ReceiptLog, *zt.AccountTokenBalanceReceipt, error) {
	...
	addLeafKvs, err := AddNewLeafOpt(statedb, ethAddress, tokenID, accountID, amount, chainAddr)
	if nil != err {
		return nil, nil, nil, err
	}
...
func GetLeafByEthAddress(db dbm.KV, ethAddress string) ([]*zt.Leaf, error) {
	...
	for _, row := range rows {
		...
		data.ChainAddr, _ = zt.DecimalAddr2Hex(data.GetChainAddr())
