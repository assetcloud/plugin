...
chain_NewAccount() {
    ...
    chain_Http "$req" ${MAIN_HTTP} '(.error|not) and (.result.acc.addr|length > 0)' "$FUNCNAME" ".result.acc.addr"
    glAddr=$RETURN_RESP
}
...
chain_SendTransaction() {
    ...
    chain_Http "$req" ${MAIN_HTTP} '(.error|not)' "Chain.SignRawTx" ".result"
    signTx=$RETURN_RESP
    ...
    gResp=$RETURN_RESP
    #返回交易
    chain_QueryTx "$RETURN_RESP" "${MAIN_HTTP}"
}
...
blackwhite_BlackwhiteCreateTx() {
    ...
    chain_Http "$req" ${MAIN_HTTP} '(.error|not)' "$FUNCNAME" ".result"
    #发送交易
    chain_SendTransaction "$RETURN_RESP" "${addr}"
    gID="${gResp}"
}
...
blackwhite_GetBlackwhiteRoundInfo() {
    ...
    chain_Http "$req" ${MAIN_HTTP} '(.error|not) and (.result.round | [has("gameID", "status", "playAmount", "playerCount", "curPlayerCount", "loop", "curShowCount", "timeout"),true] | unique | length == 1)' "$FUNCNAME"
}
...
function run_testcases() {
    ...
    #先创建账户地址
    chain_NewAccount "label188"
    gameAddr1="${glAddr}"
    chain_NewAccount "label288"
    gameAddr2="${glAddr}"
    chain_NewAccount "label388"
    gameAddr3="${glAddr}"

    #给每个账户分别转帐
    origAddr="12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv"

    chain_GetAccounts "${MAIN_HTTP}"

    #主链中相应账户需要转帐
    M_HTTP=${MAIN_HTTP//8901/8801}
    chain_SendToAddress "${origAddr}" "${gameAddr1}" 1000000000 "${M_HTTP}"
    chain_SendToAddress "${origAddr}" "${gameAddr2}" 1000000000 "${M_HTTP}"
    chain_SendToAddress "${origAddr}" "${gameAddr3}" 1000000000 "${M_HTTP}"

    #平行链相应账户需要转帐
    chain_SendToAddress "${origAddr}" "${gameAddr1}" 1000000000 "${MAIN_HTTP}"
    chain_SendToAddress "${origAddr}" "${gameAddr2}" 1000000000 "${MAIN_HTTP}"
    chain_SendToAddress "${origAddr}" "${gameAddr3}" 1000000000 "${MAIN_HTTP}"

    #给游戏合约中转帐
    chain_SendToAddress "${gameAddr1}" "${bwExecAddr}" 500000000 "${MAIN_HTTP}"
    chain_SendToAddress "${gameAddr2}" "${bwExecAddr}" 500000000 "${MAIN_HTTP}"
    chain_SendToAddress "${gameAddr3}" "${bwExecAddr}" 500000000 "${MAIN_HTTP}"
...
function main() {
    chain_RpcTestBegin blackwhite
    MAIN_HTTP="$1"
    echo "main_ip=$MAIN_HTTP"

    init
    run_testcases
    chain_RpcTestRst blackwhite "$CASE_ERR"
}
