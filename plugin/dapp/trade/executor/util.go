...
func checkAsset(cfg *types.ChainConfig, height int64, exec, symbol string) bool {
...
func checkPrice(cfg *types.ChainConfig, height int64, exec, symbol string) bool {
...
func notSameAsset(cfg *types.ChainConfig, height int64, assetExec, assetSymbol, priceExec, priceSymbol string) bool {
	if cfg.IsDappFork(height, pt.TradeX, pt.ForkTradePriceX) {
...
func createAccountDB(cfg *types.ChainConfig, height int64, db db.KV, exec, symbol string) (*account.DB, error) {
...
func createPriceDB(cfg *types.ChainConfig, height int64, db db.KV, exec, symbol string) (*account.DB, error) {
	if cfg.IsDappFork(height, pt.TradeX, pt.ForkTradePriceX) {
		// 为空默认使用 coins
		if exec == "" {
