...
contract ChainBank {
    ...
    */
    ...
    event LogBridgeTokenMint(address _token, string _symbol, uint256 _amount, address _beneficiary);

    event LogChainTokenBurn(
        address _token, string _symbol, uint256 _amount, address _ownerFrom, bytes _chainReceiver, uint256 _nonce
    );
    ...
    modifier notCreated(string memory _symbol) {
        require(!hasBridgeTokenCreated(_symbol), "The symbol has been created already");
        _;
    }
    ...
    modifier created(string memory _symbol) {
        require(hasBridgeTokenCreated(_symbol), "The symbol has not been created yet");
        _;
    }
    ...
    */
    function newChainDeposit(
        ...
        bytes32 depositID = keccak256(
            abi.encodePacked(_chainSender, _ethereumRecipient, _token, _amount, depositBurnCount.depositCount)
        );

        chainDeposits[depositID] =
            ChainDeposit(_chainSender, _ethereumRecipient, _token, _amount, true, depositBurnCount.depositCount);
    ...
    * @dev: Creates a new ChainBurn with a unique ID
    ...
    function newChainBurn(
        ...
        bytes32 burnID = keccak256(abi.encodePacked(_chainSender, _ethereumOwner, _token, _amount, nonce));

        chainBurns[burnID] = ChainBurn(_chainSender, _ethereumOwner, _token, _amount, nonce);
     ...
     */
    function deployNewBridgeToken(string memory _symbol) internal notCreated(_symbol) returns (address) {
        ...
        depositBurnCounts[newBridgeTokenAddress] = DepositBurnCount(uint256(0), uint256(0));
    ...
    function mintNewBridgeTokens(
        ...
        // Mint bridge tokens
        require(
            BridgeToken(_bridgeTokenAddress).mint(_intendedRecipient, _amount), "Attempted mint of bridge tokens failed"
        );

        newChainDeposit(_chainSender, _intendedRecipient, _bridgeTokenAddress, _amount);
    ...
    function burnChainTokens(
        ...
        // Must be whitelisted bridge token
        require(bridgeTokenWhitelist[_chainTokenAddress], "Token must be a whitelisted bridge token");
        ...
        require(depositBurnCount.burnCount + 1 > depositBurnCount.burnCount, "burn nonce is not available");
        depositBurnCount.burnCount = depositBurnCount.burnCount.add(1);
        depositBurnCounts[_chainTokenAddress] = depositBurnCount;

        newChainBurn(_chainReceiver, _from, _chainTokenAddress, _amount, depositBurnCount.burnCount);

        emit LogChainTokenBurn(
            _chainTokenAddress, bridgeTokenInstance.symbol(), _amount, _from, _chainReceiver, depositBurnCount.burnCount
        );
    }
    ...
    */
    function isLockedChainDeposit(bytes32 _id) internal view returns (bool) {
    ...
    */
    function getChainDeposit(bytes32 _id) internal view returns (bytes memory, address payable, address, uint256) {
        ChainDeposit memory deposit = chainDeposits[_id];

        return (deposit.chainSender, deposit.ethereumRecipient, deposit.bridgeTokenAddress, deposit.amount);
    }

    function getToken2address(string memory _symbol) public view created(_symbol) returns (address) {
        bytes32 symHash = keccak256(abi.encodePacked(_symbol));
        return token2address[symHash];
    }

    function getToken2addressV2(string memory _symbol) public view created(_symbol) returns (address, bool) {
        bytes32 symHash = keccak256(abi.encodePacked(_symbol));
