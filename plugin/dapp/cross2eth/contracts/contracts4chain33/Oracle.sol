...
contract Oracle {
    ...
    */
    modifier isPending(bytes32 _claimID) {
        require(
            ethereumBridge.isProphecyClaimActive(_claimID) == true, "The prophecy must be pending for this operation"
        );
        _;
    }
    ...
    */
    modifier isValidClaimType(ClaimType _claimType) {
        require(ethereumBridge.isValidClaimType(uint8(_claimType)) == true, "The claim type must be burn or lock");
        _;
    }
    ...
    * @dev: Constructor
    */
    constructor(address _operator, address _valset, address _ethereumBridge) public {
        operator = _operator;
    ...
    */
    ...
    {
        require(ethereumBridge.isProphecyClaimActive(_claimID) == true, "Can only check active prophecies");
    ...
    */
    function completeClaim(bytes32 _claimID) internal {
        ethereumBridge.completeClaim(_claimID);
    }
}
