...
contract EthereumBank {
    ...
    */
    ...
    event LogEthereumTokenBurn(
        address _token, string _symbol, uint256 _amount, address _ownerFrom, bytes _ethereumReceiver, uint256 _nonce
    );
    ...
    */
    function newEthereumDeposit(
        ...
        bytes32 depositID = keccak256(
            abi.encodePacked(_ethereumSender, _chainRecipient, _token, _amount, depositBurnCount.depositCount)
        );

        ethereumDeposits[depositID] =
            EthereumDeposit(_ethereumSender, _chainRecipient, _token, _amount, true, depositBurnCount.depositCount);
    ...
    * @dev: Creates a new EthereumBurn with a unique ID
    ...
    function newEthereumBurn(
        ...
        bytes32 burnID = keccak256(abi.encodePacked(_ethereumSender, _chainOwner, _token, _amount, nonce));

        ethereumBurns[burnID] = EthereumBurn(_ethereumSender, _chainOwner, _token, _amount, nonce);
    ...
    function deployNewBridgeToken(string memory _symbol) internal notCreated(_symbol) returns (address) {
        ...
        depositBurnWithdrawCounts[newBridgeTokenAddress] = DepositBurnWithdrawCount(uint256(0), uint256(0), uint256(0));
    ...
    function mintNewBridgeTokens(
        ...
        newEthereumDeposit(_ethereumSender, _intendedRecipient, _bridgeTokenAddress, _amount);
    ...
    function burnEthereumTokens(
        ...
        // Must be whitelisted bridge token
        require(bridgeTokenWhitelist[_ethereumTokenAddress], "Token must be a whitelisted bridge token");
        ...
        newEthereumBurn(_ethereumReceiver, _from, _ethereumTokenAddress, _amount, depositBurnCount.burnCount);
    ...
    function withdrawEthereumTokens(
        ...
        require(wdCount.withdrawCount + 1 > wdCount.withdrawCount, "withdraw nonce is not available");
    ...
    */
    function getEthereumDeposit(bytes32 _id) internal view returns (bytes memory, address payable, address, uint256) {
        EthereumDeposit memory deposit = ethereumDeposits[_id];

        return (deposit.ethereumSender, deposit.chainRecipient, deposit.bridgeTokenAddress, deposit.amount);
    }
