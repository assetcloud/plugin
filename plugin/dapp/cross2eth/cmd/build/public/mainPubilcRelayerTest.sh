...

{
    ...
    chainMultisignA="168Sn1DXnLrZHTcAM9stD6t2P49fNuJfJ9"
    chainMultisignB="13KTf57aCkVVJYNJBXBBveiA5V811SrLcT"
    chainMultisignC="1JQwQWsShTHC4zxHzbUfYQK4kRBriUQdEe"
    chainMultisignD="1NHuKqoKe3hyv52PF8XBAyaTmJWAqA2Jbb"
    chainMultisignKeyA="0xcd284cd17456b73619fa609bb9e3105e8eff5d059c5e0b6eb1effbebd4d64144"
    chainMultisignKeyB="0xe892212221b3b58211b90194365f4662764b6d5474ef2961ef77c909e31eeed3"
    chainMultisignKeyC="0x9d19a2e9a440187010634f4f08ce36e2bc7b521581436a99f05568be94dc66ea"
    chainMultisignKeyD="0x45d4ce009e25e6d5e00d8d3a50565944b2e3604aa473680a656b242d9acbff35"
    ...
    chainValidatorp="1GTxrmuWiXavhcvsaH5w9whgVxUrWsUMdV"
    chainValidatorKeyp="0xd627968e445f2a41c92173225791bae1ba42126ae96c32f28f97ff8f226e5c68"

    # 代理收币地址
    chainValidatorsp="1Hf1wnnr6XaYy5Sf3HhAfT4N8JYV4sMh9J"
    chainValidatorKeysp="0x1dadb7cbad8ea3f968cfad40ac32981def6215690618e62c48e816e7c732a8c2"
...
function updata_toml_start_bcd() {
    for name in b c d; do
        ...
        init_validator_relayer "${CLI}" "${validatorPwd}" "${chainValidatorKey}" "${ethValidatorAddrKey}"
    done
}
...
# chain lock BTY, eth burn BTY
function TestChainToEthAssets() {
    echo -e "${GRE}=========== $FUNCNAME begin ===========${NOC}"
    echo -e "${GRE}=========== chain lock BTY, eth burn BTY ===========${NOC}"
    result=$(${CLIA} ethereum balance -o "${ethTestAddr1}" -t "${ethereumBtyBridgeTokenAddr}")
    cli_ret "${result}" "balance" ".balance" "0"

    # 原来的地址金额
    result=$(${ChainCli} asset balance -a "${chainTestAddr1}" --asset_exec paracross --asset_symbol coins.bty -e "${paraName}evm" | jq -r .balance)
    is_equal "${result}" "500.0000"

    # chain lock bty
    hash=$(${ChainCli} send evm call -f 1 -a 5 -k "${chainTestAddr1}" -e "${chainBridgeBank}" -p "lock(${ethTestAddr1}, ${chainBtyERC20TokenAddr}, 500000000)")
    ...
    # chainBridgeBank 是否增加了 5
    result=$(${ChainCli} asset balance -a "${chainBridgeBank}" --asset_exec paracross --asset_symbol coins.bty -e "${paraName}evm" | jq -r .balance)
    ...
    # eth burn
    result=$(${CLIA} ethereum burn -m 3 -k "${ethTestAddrKey1}" -r "${chainReceiverAddr}" -t "${ethereumBtyBridgeTokenAddr}") #--node_addr https://ropsten.infura.io/v3/9e83f296716142ffbaeaafc05790f26c)
    cli_ret "${result}" "burn"
    ...
    # 接收的地址金额 变成了 3
    result=$(${ChainCli} asset balance -a "${chainReceiverAddr}" --asset_exec paracross --asset_symbol coins.bty -e "${paraName}evm" | jq -r .balance)
    is_equal "${result}" "3.0000"

    # chainBridgeBank 是否减少了 3
    result=$(${ChainCli} asset balance -a "${chainBridgeBank}" --asset_exec paracross --asset_symbol coins.bty -e "${paraName}evm" | jq -r .balance)
    is_equal "${result}" "2.0000"

    # eth burn 2
    result=$(${CLIA} ethereum burn -m 2 -k "${ethTestAddrKey1}" -r "${chainReceiverAddr}" -t "${ethereumBtyBridgeTokenAddr}") #--node_addr https://ropsten.infura.io/v3/9e83f296716142ffbaeaafc05790f26c)
    cli_ret "${result}" "burn"
...
function TestETH2ChainAssets() {
    echo -e "${GRE}=========== $FUNCNAME begin ===========${NOC}"
    echo -e "${GRE}=========== eth to chain 在以太坊上锁定 ETH 资产,然后在 chain 上 burn ===========${NOC}"
    result=$(${CLIA} ethereum balance -o "${ethereumBridgeBank}")
    cli_ret "${result}" "balance" ".balance" "0"

    result=$(${CLIA} ethereum lock -m 0.002 -k "${ethTestAddrKey1}" -r "${chainReceiverAddr}")
    cli_ret "${result}" "lock"
    ...
    result=$(${CLIA} chain burn -m 0.0003 -k "${chainReceiverAddrKey}" -r "${ethTestAddr2}" -t "${chainMainBridgeTokenAddr}")
    cli_ret "${result}" "burn"
    ...
    echo "check the balance on chain"
    result=$(${ChainCli} evm query -a "${chainMainBridgeTokenAddr}" -c "${chainDeployAddr}" -b "balanceOf(${chainReceiverAddr})")
    ...
    result=$(${CLIA} chain burn -m 0.0017 -k "${chainReceiverAddrKey}" -r "${ethTestAddr2}" -t "${chainMainBridgeTokenAddr}")
    cli_ret "${result}" "burn"
...
function TestETH2ChainUSDT() {
    echo -e "${GRE}=========== $FUNCNAME begin ===========${NOC}"
    echo -e "${GRE}=========== eth to chain 在以太坊上锁定 USDT 资产,然后在 chain 上 burn ===========${NOC}"
    ...
    result=$(${CLIA} chain burn -m 5 -k "${chainReceiverAddrKey}" -r "${ethReceiverAddr1}" -t "${chainUSDTBridgeTokenAddr}")
    cli_ret "${result}" "burn"
    ...
    result=$(${CLIA} chain burn -m 7 -k "${chainReceiverAddrKey}" -r "${ethReceiverAddr1}" -t "${chainUSDTBridgeTokenAddr}")
    cli_ret "${result}" "burn"
...
function offline_set_offline_token_Bty() {
    echo -e "${GRE}=========== $FUNCNAME begin ===========${NOC}"
    echo -e "${GRE}===== chain 端 configLockedTokenOfflineSave BTY ======${NOC}"
    ...
    ${Boss4xCLI} chain offline set_offline_token -c "${chainBridgeBank}" -s BTY -m ${threshold} -p ${percents} -k "${chainDeployKey}"
    chain_offline_send "chain_set_offline_token.txt"
...
function lock_bty_multisign_docker() {
    ...
    hash=$(${ChainCli} send evm call -f 1 -a "${lockAmount}" -k "${chainTestAddr1}" -e "${chainBridgeBank}" -p "lock(${ethTestAddr1}, ${chainBtyERC20TokenAddr}, ${lockAmount2})")
    check_tx "${ChainCli}" "${hash}"
    ...
    if [[ $# -eq 3 ]]; then
        ...
        result=$(${ChainCli} asset balance -a "${chainMultisignAddr}" --asset_exec paracross --asset_symbol coins.bty -e "${paraName}evm" | jq -r .balance)
...
function lockBty() {
    echo -e "${GRE}=========== $FUNCNAME begin ===========${NOC}"
    echo -e "${GRE}===== chain 端 lock BTY ======${NOC}"
    #    echo '2:#配置自动转离线钱包(bty, 100, 50%)'
...
function up_relayer_toml() {
    ...
    sed -i 's/^chainHost=.*/chainHost="http:\/\/'"${docker_chain_ip}"':8901"/' "${relaye_file}"
    sed -i 's/^chainRpcUrls=.*/chainRpcUrls=["http:\/\/'"${docker_chain_ip}"':8901","http:\/\/'"${docker_chain30_ip}"':8901","http:\/\/'"${docker_chain31_ip}"':8901","http:\/\/'"${docker_chain32_ip}"':8901"]/' "${relaye_file}"
...
function StartDockerRelayerDeploy() {
    ...
    # 设置 token 地址
    {
        ...
        offline_create_bridge_token_chain_symbol "BUSDT"
        chainUSDTBridgeTokenAddrOnBSC="${chainMainBridgeTokenAddr}"
        offline_create_bridge_token_chain_symbol "BNB"
        chainMainBridgeTokenAddrBNB="${chainMainBridgeTokenAddr}"
        offline_create_bridge_token_eth_BTY
        ...
        docker cp "${chainUSDTBridgeTokenAddrOnBSC}.abi" "${dockerNamePrefix}_ebrelayera_1":/root/${chainUSDTBridgeTokenAddrOnBSC}.abi
        docker cp "${chainMainBridgeTokenAddrETH}.abi" "${dockerNamePrefix}_ebrelayera_1":/root/${chainMainBridgeTokenAddrETH}.abi
        docker cp "${chainMainBridgeTokenAddrBNB}.abi" "${dockerNamePrefix}_ebrelayera_1":/root/${chainMainBridgeTokenAddrBNB}.abi
...
function echo_addrs() {
    ...
    echo -e "${GRE}chainBridgeBank: ${chainBridgeBank} ${NOC}"
    echo -e "${GRE}chainBridgeRegistry: ${chainBridgeRegistry} ${NOC}"
    echo -e "${GRE}chainMultisignAddr: ${chainMultisignAddr} ${NOC}"
    echo -e "${GRE}chainUSDTBridgeTokenAddr: ${chainUSDTBridgeTokenAddr} ${NOC}"
    echo -e "${GRE}chainMainBridgeTokenAddrETH: ${chainMainBridgeTokenAddrETH} ${NOC}"
    echo -e "${GRE}chainMainBridgeTokenAddrBNB: ${chainMainBridgeTokenAddrBNB} ${NOC}"
    ...
    echo -e "${GRE}XgoBridgeRegistryOnChain: ${XgoBridgeRegistryOnChain} ${NOC}"

    echo -e "${GRE}XgoChainBridgeBank: ${XgoChainBridgeBank} ${NOC}"

    echo -e "${GRE}=========== echo don't use addrs ===========${NOC}"
    echo -e "${GRE}ethDeployAddr: ${ethDeployAddr} ${NOC}"
    echo -e "${GRE}chainDeployAddr: ${chainDeployAddr} ${NOC}"
    echo -e "${GRE}chainValidatorA: ${chainValidatora} ${NOC}"
    echo -e "${GRE}chainValidatorB: ${chainValidatorb} ${NOC}"
    echo -e "${GRE}chainValidatorC: ${chainValidatorc} ${NOC}"
    echo -e "${GRE}chainValidatorD: ${chainValidatord} ${NOC}"
    ...
    echo -e "${GRE}chainTestAddr1: ${chainTestAddr1} ${NOC}"
    echo -e "${GRE}chainTestAddrKey1: ${chainTestAddrKey1} ${NOC}"
}
