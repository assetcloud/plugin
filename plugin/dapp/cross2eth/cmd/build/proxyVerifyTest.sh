...
function start_docker_ebrelayerProxy() {
    ...
    init_validator_relayer "${CLIP}" "${validatorPwd}" "${chainValidatorKeyp}" "${ethValidatorAddrKeyp}"
    sleep 20
}
...
function setWithdraw_ethereum() {
    ...
    # 在chain上的bridgeBank合约中设置proxyReceiver
    ${Boss4xCLI} chain offline set_withdraw_proxy -c "${chainBridgeBank}" -a "${chainValidatorsp}" -k "${chainDeployKey}" -n "set_withdraw_proxy:${chainValidatorsp}"
    chain_offline_send "set_withdraw_proxy.txt"
}
...
# eth to chain 在以太坊上锁定 ETH 资产,然后在 chain 上 withdraw
function TestETH2ChainAssets_proxy() {
    echo -e "${GRE}=========== $FUNCNAME begin ===========${NOC}"
    echo -e "${GRE}=========== eth to chain 在以太坊上锁定 ETH 资产,然后在 chain 上 withdraw ===========${NOC}"
    ...
    echo -e "${IYellow} chainReceiverAddr chain 端 lock 后接收地址初始金额 ${NOC}"
    chainRBalancebf=$(${ChainCli} evm query -a "${chainMainBridgeTokenAddr}" -c "${chainDeployAddr}" -b "balanceOf(${chainReceiverAddr})")

    echo -e "${IYellow} chainValidatorsp chain 代理地址初始金额 ${NOC}"
    chainVspBalancebf=$(${ChainCli} evm query -a "${chainMainBridgeTokenAddr}" -c "${chainDeployAddr}" -b "balanceOf(${chainValidatorsp})")

    echo -e "${IYellow} lock ${NOC}"
    result=$(${CLIP} ethereum lock -m "${lockAmount1}" -k "${ethTestAddrKey1}" -r "${chainReceiverAddr}")
    cli_ret "${result}" "lock"
    ...
    echo -e "${IYellow} chainReceiverAddr chain 端 lock 后接收地址 lock 后金额 ${NOC}"
    result=$(${ChainCli} evm query -a "${chainMainBridgeTokenAddr}" -c "${chainDeployAddr}" -b "balanceOf(${chainReceiverAddr})")

    let chainRBalancelock=lockAmount1*le8+chainRBalancebf
    is_equal "${result}" "${chainRBalancelock}"

    echo -e "${IYellow} chainValidatorsp chain 代理地址 lock 后金额 ${NOC}"
    result=$(${ChainCli} evm query -a "${chainMainBridgeTokenAddr}" -c "${chainDeployAddr}" -b "balanceOf(${chainValidatorsp})")
    is_equal "${result}" "${chainVspBalancebf}"
    ...
    result=$(${CLIP} chain withdraw -m "${lockAmount1}" -k "${chainReceiverAddrKey}" -r "${ethTestAddr2}" -t "${chainMainBridgeTokenAddr}")
    ...
    echo -e "${IYellow} chainReceiverAddr chain 端 lock 后接收地址 withdraw 后金额 ${NOC}"
    result=$(${ChainCli} evm query -a "${chainMainBridgeTokenAddr}" -c "${chainDeployAddr}" -b "balanceOf(${chainReceiverAddr})")
    is_equal "${result}" "${chainRBalancebf}"

    echo -e "${IYellow} chainValidatorsp chain 代理地址 withdraw 后金额 ${NOC}"
    result=$(${ChainCli} evm query -a "${chainMainBridgeTokenAddr}" -c "${chainDeployAddr}" -b "balanceOf(${chainValidatorsp})")
    let chainVspBalancewithdraw=lockAmount1*le8+chainVspBalancebf
    is_equal "${result}" "${chainVspBalancewithdraw}"
...
# eth to chain 在以太坊上锁定 ETH 资产,然后在 chain 上 withdraw
function TestETH2ChainAssets_proxy_excess() {
...
function TestETH2ChainUSDT_proxy() {
    echo -e "${GRE}=========== $FUNCNAME begin ===========${NOC}"
    echo -e "${GRE}=========== eth to chain 在以太坊上锁定 USDT 资产,然后在 chain 上 withdraw ===========${NOC}"
    ...
    echo -e "${IYellow} chainReceiverAddr chain 端 lock 后接收地址初始金额 ${NOC}"
    chainRBalancebf=$(${ChainCli} evm query -a "${chainUSDTBridgeTokenAddr}" -c "${chainTestAddr1}" -b "balanceOf(${chainReceiverAddr})")

    echo -e "${IYellow} chainValidatorsp chain 代理地址初始金额 ${NOC}"
    chainVspBalancebf=$(${ChainCli} evm query -a "${chainUSDTBridgeTokenAddr}" -c "${chainTestAddr1}" -b "balanceOf(${chainValidatorsp})")

    echo -e "${IYellow} ETH 这端 lock $lockAmount1 个 USDT ${NOC}"
    result=$(${CLIP} ethereum lock -m "${lockAmount1}" -k "${ethTestAddrKey1}" -r "${chainReceiverAddr}" -t "${ethereumUSDTERC20TokenAddr}")
    cli_ret "${result}" "lock"
    ...
    echo -e "${IYellow} chainValidatorsp chain 代理地址 lock 后金额 ${NOC}"
    result=$(${ChainCli} evm query -a "${chainUSDTBridgeTokenAddr}" -c "${chainTestAddr1}" -b "balanceOf(${chainValidatorsp})")
    is_equal "${result}" "${chainVspBalancebf}"
    ...
    echo -e "${IYellow} withdraw ${NOC}"
    result=$(${CLIP} chain withdraw -m "${lockAmount1}" -k "${chainReceiverAddrKey}" -r "${ethReceiverAddr1}" -t "${chainUSDTBridgeTokenAddr}")
    cli_ret "${result}" "withdraw"
...
function TestETH2ChainUSDT_proxy_excess() {
    echo -e "${GRE}=========== $FUNCNAME begin ===========${NOC}"
    echo -e "${GRE}=========== eth to chain 在以太坊上锁定 USDT 资产,然后在 chain 上 withdraw ===========${NOC}"
...
function TestProxy() {
    ...
    TestETH2ChainAssets_proxy 20
    TestETH2ChainAssets_proxy 30
    TestETH2ChainAssets_proxy_excess 100

    ${CLIP} ethereum token token_transfer -k "${ethTestAddrKey1}" -m 500 -r "${ethValidatorAddrp}" -t "${ethereumUSDTERC20TokenAddr}"
    TestETH2ChainUSDT_proxy 20
    TestETH2ChainUSDT_proxy 40
    TestETH2ChainUSDT_proxy_excess 100
}
...
function parallel_test() {
    ...
    for ((i = 0; i < 10; i++)); do
        parallel --jobs 20 ${CLIA} ethereum lock -m 0.005 -k {} -r "${chainReceiverAddr}" ::: 0x2809477ede1261da21270096776ba7dc68b89c9df5f029965eaa5fe7f0b80697 0x0c61f5a879d70807686e43eccc1f52987a15230ae0472902834af4d1933674f2 0x1385016736f7379884763f4a39811d1391fa156a7ca017be6afffa52bb327695 0x4ae589fe3837dcfc90d1c85b8423dc30841525cbebc41dfb537868b0f8376bbf 0x62ca4122aac0e6f35bed02fc15c7ddbdaa07f2f2a1821c8b8210b891051e3ee9 0x355b876d7cbcb930d5dfab767f66336ce327e082cbaa1877210c1bae89b1df71 0x72dff1c863631208a3d4f67a5fb0b7ebe69f827a75f332e2069dc7c825cb2202 0xecbc20b02e1ffd321e31c2a6d7d35a69715ba43ef2b0048a27de4f67b8249bde 0x1649955b3f2852a9cd71e50237b5a6f717539cffbe336bfcd95eb19a1b5c6f1b 0xd3064a91f01a60b0e3d92d08fc8be144a61a2a4a7780827727cd3804a66d31bd 0x4bbe8e10a0987b6d56c505dfe41f74d357ac8211e91c4a179f9c5e38c181aaf0 0xc2d896509c89c56d2365f40c5da5680174314884118fea024013818f792fcd64 0x56e4f5884dbd4248e649bb5163815dd6fcae7a656f43419becd183e49fe2b514 0x6ae0d6e5f14c1719f170100b6a84f3d7be14c2623404191d854ef98739e813a6 0x0e8b158dae56bc58c69ea9cac8737317dc47d2fb50d40bea4f1629414a8f7846 0x3d5ac5f963568544b8b855c8950030ec60cd0a0e9a5a8be725e790c625404fcd 0x4cca1a474c3a9789fd11987d19ae13a3553c47c7b441e77739af5b2d48f01371 0x8f23fc9a39795a1687e4f756312015d96b448829c32114981038ea74438361a0 0xbf667b0227522bd3fd220e748c43d5529e323eca9bcb005ac58e6f2431577b28 0x2e81ad3c49763fa9c0ea79b27e6cf584e6e073b43524c151ec1ade03706c1a00
        parallel --jobs 20 ${CLIA} chain burn_increase -m {} -k "${chainReceiverAddrKey}" -r "${ethTestAddr2}" -t "${chainMainBridgeTokenAddr}" ::: 0.003 0.004 0.005 0.006 0.007 0.003 0.004 0.005 0.006 0.007 0.003 0.004 0.005 0.006 0.007 0.003 0.004 0.005 0.006 0.007
    done
