...
contract GoAssetBank {
    ...
    */
    ...
    event LogGoAssetTokenBurn(
        address _token, string _symbol, uint256 _amount, address _ownerFrom, address _goAssetReceiver, uint256 _nonce
    );
    ...
    */
    function newGoAssetDeposit(address _goAssetSender, address payable _chainRecipient, address _token, uint256 _amount)
        internal
        returns (bytes32)
    {
        ...
        bytes32 depositID =
            keccak256(abi.encodePacked(_goAssetSender, _chainRecipient, _token, _amount, depositBurnCount.depositCount));

        goAssetDeposits[depositID] =
            GoAssetDeposit(_goAssetSender, _chainRecipient, _token, _amount, true, depositBurnCount.depositCount);
    ...
    * @dev: Creates a new GoAssetBurn with a unique ID
    ...
    function newGoAssetBurn(
        ...
        bytes32 burnID = keccak256(abi.encodePacked(_goAssetSender, _chainOwner, _token, _amount, nonce));

        goAssetBurns[burnID] = GoAssetBurn(_goAssetSender, _chainOwner, _token, _amount, nonce);
    ...
    function mintNewBridgeTokens(
        ...
        newGoAssetDeposit(_goAssetSender, _intendedRecipient, _bridgeTokenAddress, _amount);
    ...
    function burnGoAssetTokens(
        ...
        // Must be whitelisted bridge token
        require(bridgeTokenWhitelist[_goAssetTokenAddress], "Token must be a whitelisted bridge token");
        ...
        newGoAssetBurn(_goAssetReceiver, _from, _goAssetTokenAddress, _amount, depositBurnCount.burnCount);
    ...
    */
    function isLockedGoAssetDeposit(bytes32 _id) internal view returns (bool) {
    ...
    */
    function getGoAssetDeposit(bytes32 _id) internal view returns (address, address payable, address, uint256) {
        GoAssetDeposit memory deposit = goAssetDeposits[_id];

        return (deposit.goAssetSender, deposit.chainRecipient, deposit.bridgeTokenAddress, deposit.amount);
    }
