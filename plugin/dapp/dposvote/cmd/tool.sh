...
function impKey() {
    key=$1
    lab=$2
    ./chain-cli --rpc_laddr="${rcpAddr}" account import_key -k "${key}" -l "${lab}"
}
...
function trans() {
    ...
    tx=$(./chain-cli --rpc_laddr="${rcpAddr}" coins transfer -a "${coins}" -t "${dst}")
    echo "${tx}"
    sig=$(./chain-cli --rpc_laddr="${rcpAddr}" wallet sign -a "${src}" -d "${tx}")
    echo "${sig}"
    ./chain-cli --rpc_laddr="${rcpAddr}" wallet send -d "${sig}"
    sleep 3
    ./chain-cli --rpc_laddr="${rcpAddr}" account balance -a "${dst}"
}

function init() {
    seed=$(./chain-cli --rpc_laddr=${rcpAddr} seed generate -l 0)
    echo "${seed}"
    echo "save seed..."
    ./chain-cli --rpc_laddr="${rcpAddr}" seed save -s "${seed}" -p zzh123456
    sleep 1

    echo "unlock wallet..."
    ./chain-cli --rpc_laddr="${rcpAddr}" wallet unlock -p zzh123456
    sleep 1
...
function send_exec() {
    addr=$1
    coins=$2
    tx=$(./chain-cli --rpc_laddr="${rcpAddr}" coins send_exec -a "${coins}" -e dpos)
    sig=$(./chain-cli --rpc_laddr="${rcpAddr}" wallet sign -a "${addr}" -d "${tx}")
    ./chain-cli --rpc_laddr="${rcpAddr}" wallet send -d "${sig}"
}
...
function reg() {
    ...
    tx=$(./chain-cli --rpc_laddr="${rcpAddr}" dpos regist -a "${addr}" -i "${ip}" -k "${key}")
    sig=$(./chain-cli --rpc_laddr="${rcpAddr}" wallet sign -a "${addr}" -d "${tx}")
    ./chain-cli --rpc_laddr="${rcpAddr}" wallet send -d "$sig"
    sleep 3
}
...
function reReg() {
    ...
    tx=$(./chain-cli --rpc_laddr="${rcpAddr}" dpos reRegist -a "${addr}" -i "${ip}" -k "${key}")
    sig=$(./chain-cli --rpc_laddr="${rcpAddr}" wallet sign -a "${addr}" -d "${tx}")
    ./chain-cli --rpc_laddr="${rcpAddr}" wallet send -d "${sig}"
    sleep 3
}
function cancelReg() {
    ...
    tx=$(./chain-cli --rpc_laddr="${rcpAddr}" dpos cancelRegist -a "${addr}" -k "${key}")
    sig=$(./chain-cli --rpc_laddr="${rcpAddr}" wallet sign -a "${addr}" -d "${tx}")
    ./chain-cli --rpc_laddr="${rcpAddr}" wallet send -d "${sig}"
    sleep 3
}
function vote() {
    ...
    tx=$(./chain-cli --rpc_laddr="${rcpAddr}" dpos vote -k "${key}" -v "${votes}" -a "${addr}")
    sig=$(./chain-cli --rpc_laddr="${rcpAddr}" wallet sign -a "${addr}" -d "${tx}")
    ./chain-cli --rpc_laddr="${rcpAddr}" wallet send -d "${sig}"
    sleep 3
}
function cancelVote() {
    ...
    tx=$(./chain-cli --rpc_laddr="${rcpAddr}" dpos cancelVote -k "${key}" -i "${index}")
    sig=$(./chain-cli --rpc_laddr="${rcpAddr}" wallet sign -a "${addr}" -d "${tx}")
    ./chain-cli --rpc_laddr="${rcpAddr}" wallet send -d "${sig}"
    sleep 3
}
function regM() {
    ...
    tx=$(./chain-cli --rpc_laddr="${rcpAddr}" dpos vrfMRegist -k "${key}" -c "${cycle}" -m "${m}")
    sig=$(./chain-cli --rpc_laddr="${rcpAddr}" wallet sign -a "${addr}" -d "${tx}")
    ./chain-cli --rpc_laddr="${rcpAddr}" wallet send -d "${sig}"
    sleep 3
}
...
function regRP() {
    ...
    tx=$(./chain-cli --rpc_laddr="${rcpAddr}" dpos vrfRPRegist -k "${key}" -c "${cycle}" -r "${r}" -p "${p}")
    sig=$(./chain-cli --rpc_laddr="${rcpAddr}" wallet sign -a "${addr}" -d "${tx}")
    ./chain-cli --rpc_laddr="${rcpAddr}" wallet send -d "${sig}"
    sleep 3
}
...
function recordCB() {
    ...
    tx=$(./chain-cli --rpc_laddr="${rcpAddr}" dpos cbRecord -k "${key}" -c "${cycle}" -m "${height}" -s "${hash}")
    sig=$(./chain-cli --rpc_laddr="${rcpAddr}" wallet sign -a "${addr}" -d "${tx}")
    ./chain-cli --rpc_laddr="${rcpAddr}" wallet send -d "$sig"
    sleep 3
}
...
function qtx() {
    tx=$1
    ./chain-cli --rpc_laddr="${rcpAddr}" tx query -s "${tx}"
}
function qn() {
    result=$(./chain-cli --rpc_laddr="${rcpAddr}" dpos candidatorQuery -t topN -n "$1")
    echo "$result"
}

function qk() {
    ./chain-cli --rpc_laddr="${rcpAddr}" dpos candidatorQuery -t pubkeys -k "$1"
}

function qv() {
    ./chain-cli --rpc_laddr="${rcpAddr}" dpos voteQuery -a "$1" -k "$2"
}
function qvrf() {
    type=$1
    cycle=$2
    ./chain-cli --rpc_laddr="${rcpAddr}" dpos vrfQuery -t "$type" -c "$cycle"
}

function qvrfn() {
    cycle=$1
    ./chain-cli --rpc_laddr="${rcpAddr}" dpos vrfQuery -t "topN" -c "$cycle"
}

function qvrfk() {
    cycle=$2
    keys=$1
    ./chain-cli --rpc_laddr="${rcpAddr}" dpos vrfQuery -t "pubkeys" -c "$cycle" -k "${keys}"
}
...
function qtopn() {
    version=$1
    ./chain-cli --rpc_laddr="${rcpAddr}" dpos topNQuery -v "${version}"
}
...
function qcb() {
    ...
    param=$2
    if [ "${type}" == "cycle" ]; then
        ./chain-cli --rpc_laddr="${rcpAddr}" dpos cbQuery -t "cycle" -c "${param}"
    elif [ "${type}" == "height" ]; then
        ./chain-cli --rpc_laddr="${rcpAddr}" dpos cbQuery -t "height" -m "${param}"
    elif [ "${type}" == "hash" ]; then
        ./chain-cli --rpc_laddr="${rcpAddr}" dpos cbQuery -t "hash" -s "${param}"
    fi
}
