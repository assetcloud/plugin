...
function init() {
    ...
    chain_ImportPrivkey "${beneficiary_key}" "${beneficiary}" "unfreeze_beneficiary" "${MAIN_HTTP}"
    chain_ImportPrivkey "${owner_key}" "${owner}" "unfreeze_owner" "${MAIN_HTTP}"

    chain_applyCoins "${beneficiary}" 10000000000 "${MAIN_HTTP}"
    chain_applyCoins "${owner}" 10000000000 "${MAIN_HTTP}"
    ...
    if [ "$ispara" == true ]; then
        ...
        chain_applyCoins "${owner}" 10000000000 "${main_ip}"
    fi
    ...
    chain_SendToAddress "$owner" "$exec_addr" 5000000000 "${MAIN_HTTP}"
    chain_SendToAddress "$beneficiary" "$exec_addr" 5000000000 "${MAIN_HTTP}"
    chain_BlockWait 1 "${MAIN_HTTP}"
}
...
function CreateRawUnfreezeCreate() {
    ...
    chain_SignAndSendTx "$RETURN_RESP" "$owner_key" "${MAIN_HTTP}"
    query_unfreezeID
}
...
function rpc_test() {
    chain_RpcTestBegin unfreeze
    MAIN_HTTP="$1"
    echo "main_ip=$MAIN_HTTP"

    init
    run_testcases
    chain_RpcTestRst unfreeze "$CASE_ERR"
}
