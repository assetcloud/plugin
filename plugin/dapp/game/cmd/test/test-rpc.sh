...
function chain_GetExecAddr() {
    #获取GAME合约地址
    req='{"method":"Chain.ConvertExectoAddr","params":[{"execname":"'"$1"'"}]}'
    chain_Http "$req" ${MAIN_HTTP} '(.error|not) and (.result != null)' "$FUNCNAME"
}
...
function CreateGameTx() {
    ...
    chain_SignAndSendTx "${RETURN_RESP}" "${PRIVA_A}" "${MAIN_HTTP}"
    GAME_ID=$RAW_TX_HASH
...
function MatchGameTx() {
    ...
    chain_Http "$req" ${MAIN_HTTP} '(.error|not) and (.result != null)' "MatchGame createRawTx" ".result"
    chain_SignAndSendTx "${RETURN_RESP}" "${PRIVA_B}" "${MAIN_HTTP}"
...
function CloseGameTx() {
    ...
    chain_Http "$req" ${MAIN_HTTP} '(.error|not) and (.result != null)' "CloseGame createRawTx" ".result"
    chain_SignAndSendTx "${RETURN_RESP}" "${PRIVA_A}" "${MAIN_HTTP}"
...
function CancleGameTx() {
    ...
    chain_Http "$req" ${MAIN_HTTP} '(.error|not) and (.result != null)' "CancleGame createRawTx" ".result"
    chain_SignAndSendTx "${RETURN_RESP}" "${PRIVA_A}" "${MAIN_HTTP}"
...
function QueryGameByStatus() {
    ...
    chain_Http "$req" ${MAIN_HTTP} '(.error|not)' "$FUNCNAME" ".result.games"
}
...
function init() {
    ...
    chain_ImportPrivkey "$PRIVA_A" "16Z3haNPQd9wrnFDw19rtpbgnN2xynNT9f" "game1" "${main_ip}"
    chain_ImportPrivkey "$PRIVA_B" "16GXRfd9xj3XYMDti4y4ht7uzwoh55gZEc" "game2" "$main_ip"
    ...
    if [ "$ispara" == false ]; then
        ...
        chain_applyCoins "$ACCOUNT_B" 12000000000 "${main_ip}"
        chain_QueryBalance "${ACCOUNT_B}" "$main_ip"
    else
        chain_applyCoins "$ACCOUNT_A" 1000000000 "${main_ip}"
        chain_QueryBalance "${ACCOUNT_A}" "$main_ip"

        chain_applyCoins "$ACCOUNT_B" 1000000000 "${main_ip}"
        chain_QueryBalance "${ACCOUNT_B}" "$main_ip"
        local para_ip="${MAIN_HTTP}"
        chain_ImportPrivkey "$PRIVA_A" "16Z3haNPQd9wrnFDw19rtpbgnN2xynNT9f" "game1" "$para_ip"
        chain_ImportPrivkey "$PRIVA_B" "16GXRfd9xj3XYMDti4y4ht7uzwoh55gZEc" "game2" "$para_ip"

        chain_applyCoins "$ACCOUNT_A" 12000000000 "${para_ip}"
        chain_QueryBalance "${ACCOUNT_A}" "$para_ip"
        chain_applyCoins "$ACCOUNT_B" 12000000000 "${para_ip}"
        chain_QueryBalance "${ACCOUNT_B}" "$para_ip"
    fi

    chain_SendToAddress "${ACCOUNT_B}" "$game_addr" 5000000000 "${MAIN_HTTP}"
    chain_QueryExecBalance "${ACCOUNT_B}" "game" "$MAIN_HTTP"
    chain_SendToAddress "${ACCOUNT_A}" "$game_addr" 5000000000 "${MAIN_HTTP}"
    chain_QueryExecBalance "${ACCOUNT_A}" "game" "$MAIN_HTTP"
...
function main() {
    chain_RpcTestBegin game
    local ip=$1
    MAIN_HTTP=$ip

    init
    run_test "$MAIN_HTTP"
    chain_RpcTestRst game "$CASE_ERR"
}
