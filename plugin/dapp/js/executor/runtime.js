...
function kvcreator(dbtype) {
    ...
    if (dbtype == "exec" || dbtype == "init") {
		...
		throw new Error("chain.js: dbtype error")
	}
}
...
function callcode(context, f, args, loglist) {
    ...
    if (farr.length !=  2) {
        throw new Error("chain.js: invalid function name format")
    }
    var prefix = farr[0]
    var funcname = farr[1]
    var runobj = {}
    ...
    if (!Array.isArray(loglist)) {
        throw new Error("chain.js: loglist must be array")
    }
    ...
    if (prefix == "exec") {
        ...
        throw new Error("chain.js: invalid function prefix format")
    }
    var arg = JSON.parse(args)
    if (typeof runobj[funcname] != "function") {
        throw new Error("chain.js: invalid function name not found->" + funcname)
    }
