...
ticket_CreateBindMiner() {
    ...
    chain_Http "$req" ${MAIN_HTTP} '(.error|not)' "$FUNCNAME" ".result.txHex"
    chain_SignAndSendTx "$RETURN_RESP" "${returnPriv}" ${MAIN_HTTP}
}

ticket_SetAutoMining() {
    flag=$1
    req='{"method":"ticket.SetAutoMining","params":[{"flag":'"$flag"'}]}'
    chain_Http "$req" ${MAIN_HTTP} '(.error|not) and (.result.isOK == true)' "$FUNCNAME"
}

ticket_GetTicketCount() {
    chain_Http '{"method":"ticket.GetTicketCount","params":[{}]}' ${MAIN_HTTP} '(.error|not) and (.result > 0)' "$FUNCNAME"
}
...
ticket_TicketInfos() {
    ...
    minerAddr=$2
    returnAddr=$3
    req='{"method":"Chain.Query","params":[{"execer":"ticket","funcName":"TicketInfos","payload":{"ticketIds":["'"$tid"'"]}}]}'
...
ticket_TicketList() {
    ...
    returnAddr=$2
    status=$3
    req='{"method":"Chain.Query","params":[{"execer":"ticket","funcName":"TicketList","payload":{"addr":"'"$minerAddr"'", "status":'"$status"'}}]}'
...
ticket_MinerAddress() {
    returnAddr=$1
    minerAddr=$2
    req='{"method":"Chain.Query","params":[{"execer":"ticket","funcName":"MinerAddress","payload":{"data":"'"$returnAddr"'"}}]}'
...
ticket_MinerSourceList() {
    minerAddr=$1
    returnAddr=$2
    req='{"method":"Chain.Query","params":[{"execer":"ticket","funcName":"MinerSourceList","payload":{"data":"'"$minerAddr"'"}}]}'
...
ticket_RandNumHash() {
    hash=$1
    blockNum=$2
    req='{"method":"Chain.Query","params":[{"execer":"ticket","funcName":"RandNumHash","payload":{"hash":"'"$hash"'", "blockNum":'"$blockNum"'}}]}'
    chain_Http "$req" ${MAIN_HTTP} '(.error|not) and (.result.hash != null)' "$FUNCNAME"
}
...
function main() {
    ...
    chain_RpcTestRst Ticket "$CASE_ERR"
}
